...
public class EstimatePi_2 extends JPanel implements ActionListener {
    ...
    private final JLabel piEstimateLabel;  // A label for showing the current estimate of pi.
    private final JLabel countLabel;       // A label for showing the number of trials.
    
    private final JButton runPauseButton;  // Button to control the thread.  Clicking this
                                     // button will pause the thread if it is running
                                     // and will restart it if it is paused.
    
    private final ComputationThread runner;  // The thread that does the computation.
